...
function buildUserPrompt(
  ...
  for (let i = 0; i < videos.length; i++) {
    const v = videos[i];
    const transcript = transcripts.get(v.videoId) ?? "(no transcript available)";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const t1 = Date.now();
    console.log(`[Saga Analyze] DB query: ${t1 - t0}ms, found ${dbRows.length}/${videoIds.length} cached transcripts`);
    ...
    for (const row of dbRows) {
      ...
      if (text) {
        allTranscripts.set(row.videoId, text);
      }
    }
    ...
    if (toFetch.length > 0 && toFetch.length <= 5) {
      console.log(`[Saga Analyze] Fetching ${toFetch.length} missing transcripts from YouTube`);
      const t2 = Date.now();
      const freshTranscripts = await fetchTranscriptBatch(toFetch, 1000);
      console.log(`[Saga Analyze] Transcript fetch: ${Date.now() - t2}ms for ${toFetch.length} videos, got ${freshTranscripts.size} results`);

      for (const [id, text] of freshTranscripts) {
        allTranscripts.set(id, text);
      }
    ...
    } else if (toFetch.length > 5) {
      console.log(`[Saga Analyze] Skipping ${toFetch.length} missing transcripts (too many — run transcript sync first)`);
    }

    console.log(`[Saga Analyze] Total transcript phase: ${Date.now() - t0}ms, ${allTranscripts.size}/${videoIds.length} have transcripts`);

    const userPrompt = buildUserPrompt(videos, allTranscripts, overlapContext, knownSagaNames);

    const tAi = Date.now();
    let aiText: string;
    try {
      aiText = await callAI(userPrompt);
    } catch (err) {
    ...
    console.log(`[Saga Analyze] AI call: ${Date.now() - tAi}ms, response length: ${aiText.length} chars`);

    const { segments, tailContext } = parseAiResponse(aiText, videos);

    const response: AnalyzeResponse = { segments, tailContext };
